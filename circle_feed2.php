...
<script type="text/javascript">
	...
	<div class="container">
		<div class="row" style="margin-top: 60px;">
			...
			foreach($db->query("SELECT * FROM circle_list WHERE id='$circle_id'") as $row){
				if($_SESSION['user']==$row['creater']){
					$check=0;
				}
			}
			foreach($db->query("SELECT * FROM circle_mem WHERE circle_id='$circle_id'") as $row){
				if($_SESSION['user']==$row['member']){
					$check=0;
				}
			}
			if($check==1){
				die("<h1>You are not the member of this circle</h1>");
			}
